...
@login_required(login_url='auth/logar/')
def gerar_pdf(request, id_paciente):
    paciente = get_object_or_404(Pacientes, id=id_paciente)
    refeicoes = Refeicao.objects.filter(paciente=paciente).order_by('horario')
    opcao = Opcao.objects.all()

    # Renderiza o template como string
    html_string = render_to_string('relatorio_dieta.html', {'paciente': paciente, 'refeicoes': refeicoes, 'opcao': opcao})
    ...
    response["Content-Disposition"] = f'attachment; filename="Dieta_{paciente.nome}.pdf"'
